...
print(f"Atom group: {ATOM_GROUP_TO_ANALYZE_1} and {ATOM_GROUP_TO_ANALYZE_2}")
...
for i in range(1, NUM_ENSEMBLES + 1):
    ...
    try:
        ...
        atoms_name_1 = u.select_atoms(ATOM_GROUP_TO_ANALYZE_1)
        atoms_name_2 = u.select_atoms(ATOM_GROUP_TO_ANALYZE_2)
        
        if len(atoms_name_1) < 2:
            print(f"  WARNING: Fewer than 2 atoms found for '{ATOM_GROUP_TO_ANALYZE_1}'. Skipping RDF calculation.")
            continue
        
        if len(atoms_name_2) < 2 :
            print(f"  WARNING: Fewer than 2 atoms found for '{ATOM_GROUP_TO_ANALYZE_2}'. Skipping RDF calculation.")
            continue
        
        num_frames = len(u.trajectory)
        start_frame = int(num_frames * (SKIP_PERCENT / 100.0))

        rdf_analysis = rdf.InterRDF(atoms_name_1, atoms_name_2, 
                                    nbins=NBINS, 
                                    range=RANGE, 
                                    exclusion_block=(1, 1))
...
if all_rdfs:
    ...
    print(f"g(r) results have been saved to {output_filename}")

    # --- Plotting ---
    plt.style.use('seaborn-v0_8-whitegrid')
    fig, ax = plt.subplots(figsize=(10, 7))
    
    ax.plot(distance_bins, avg_rdf, color='royalblue', lw=2.5, label=f'Ensemble Average g({atoms_name_1}-{atoms_name_2})')
    ...
    plot_filename = f'rdf_{atoms_name_1}-{atoms_name_2}.png'
