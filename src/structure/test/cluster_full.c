...
int main(int argc, char *argv[]) {
    if (argc != 7) {
        printf("USAGE: %s <input.xyz> <cluster_size.dat> <na_coord.dat> <cluster_dist.dat> <vanHove_poly.dat> <vanHove_iso.dat>\n", argv[0]);
        exit(1);
    }
...
void classify_na_ions() {
    printf("\tNow Classifying Na+ ions...\n");

    // na_classification[t][na_idx]
    ...
    for (int t = 0; t < numTraj; t++) {
        ...
        if (!na_classification[t]) {
            fprintf(stderr, "Error: Failed to allocate na_classification[t]\n");
            exit(1);
        }
        ...
        for (int c = 0; c <= max_id; c++) {
            ...
            // Isolated TaCl6 Definition
            if (cluster_Ta_count[c] == 1 &&
                cluster_O_count[c] == 0 &&
...
void calculate_van_hove_classified(const char *poly_filename, const char *iso_filename) {
    printf("\tNow Gs(r,t) for classified Na+...\n");
